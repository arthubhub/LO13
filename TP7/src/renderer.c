...
float CalculerId(int sommet_id){
    int virt_base, real_base;
    float Id_sommet;

    virt_base = sommet_id; 
    real_base = 3 * virt_base; // la ou se trouve la première normale

    float normal_x = msh.normal_v[real_base]    * ogl.normales_Current_Factor[0]; // normale x
    float normal_y = msh.normal_v[real_base + 1]* ogl.normales_Current_Factor[1]; // normale y
    float normal_z = msh.normal_v[real_base + 2]* ogl.normales_Current_Factor[2];  // normale z

    Id_sommet = (normal_x + normal_y + normal_z) ;
    

}

void TracerLigneCible(float seuil) {
    float Points[9];
    int Id = 0;
    float Id_a, Id_b, Id_c;
    ...
    for (int i = 0; i < msh.number_of_triangles; i++) {
        ...
        // calculer les Id à chaque sommet :
        Id_a = CalculerId(k+0);
        Id_b = CalculerId(k+1);
        Id_c = CalculerId(k+2);
...
void TracerDiffusion(void){
    float Id_cible;
    for (int i = 1; i <= ogl.nb_lignes_diffusion; ++i) {
        Id_cible = (float)i / (float)ogl.nb_lignes_diffusion;
